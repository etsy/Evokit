...
impl Scorer for MinScorer {
    /// gets the minimum value across the docs
...
mod tests {
    ...
    fn test_min_scorer() {
...

        {
            ...
            let scorer = MinScorer::new(&param);
    ...
    fn test_min_scorer_normalize() {
...

        {
            let param = AtKScoringParameters {
                ...
                k: Some(3),
                normalize: Some(true),
                opt_goal: Some(OptimizationGoal::Minimize),
            };
            let scorer = MinScorer::new(&param);
            let score_and_logger = scorer.score(&scores);
        ...
        }
        {
            ...
            let scorer = MinScorer::new(&param);
            let score_and_logger = scorer.score(&scores);
            assert_eq!(score_and_logger.0, 0.90909094)
        }
    }
}
